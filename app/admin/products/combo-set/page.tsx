...
export default function ComboSetPage() {
  ...
  const fetchComboSetProducts = async () => {
    try {
      const response = await fetch('/api/admin/products')
      if (response.ok) {
        const data = await response.json()
        const comboSetProducts = data.products.filter((p: Product) => p.category === 'combo-set')
        setProducts(comboSetProducts)
      }
    } catch (error) {
      console.error('Failed to fetch combo set products:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <div className="ml-3 text-gray-600">Loading combo sets...</div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-800">💎 COMBO SET Management</h1>
            <p className="text-gray-600">Manage complete jewelry sets with matching pieces</p>
          </div>
        </div>
        <Link href="/admin/products/combo-set/add">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <tr key={product._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-2">
                      <Link href={`/admin/products/combo-set/view/${product._id}`}>
                        <button 
        ...
        {paginatedProducts.length === 0 && (
          <div className="text-center py-12">
            ...
            <div className="text-gray-500 mb-2">No combo sets found</div>
            <p className="text-sm text-gray-400">Create your first combo set to get started</p>
            <Link href="/admin/products/combo-set/add">
      ...
      {filteredProducts.length > 0 && (
        <div className="flex items-center justify-between text-sm text-gray-600">
          <span>Showing {startIndex + 1}-{Math.min(startIndex + productsPerPage, filteredProducts.length)} of {filteredProducts.length} combo sets</span>
          <div className="flex items-center space-x-2">
            <button 
