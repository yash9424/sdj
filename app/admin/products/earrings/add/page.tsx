...
export default function AddEarringsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append('category', 'earrings')
      ...
      formDataToSend.append('productDetails', JSON.stringify({
        earrings: {
          ...
          dropLength: formData.dropLength
        }
      }))
      ...
      if (response.ok) {
        setNotification({
          show: true,
          type: 'success',
          message: 'Earrings added successfully!'
        })
        setTimeout(() => {
          window.location.href = '/admin/products/earrings'
        }, 1500)
      } else {
        setNotification({
          show: true,
          type: 'error',
          message: 'Failed to add earrings'
        })
      }
    } catch (error) {
      console.error('Error adding earrings:', error)
      setNotification({
        show: true,
        type: 'error',
        message: 'Error adding earrings'
      })
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        <Link href="/admin/products/earrings">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-800">💍 Add New Earrings</h1>
          <p className="text-gray-600">Create elegant earrings for every occasion</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="mb-8">
          <h2 className="text-xl font-bold text-gray-800 mb-4 pb-2 border-b border-green-200">Main Product Information</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Enter earrings name"
              />
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                placeholder="Describe the earrings"
              />
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                placeholder="e.g., Stud, Drop, Hoop"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                value={formData.dropLength}
                onChange={(e) => setFormData({...formData, dropLength: e.target.value})}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
        ...
        <div className="flex space-x-4 pt-6 border-t border-gray-200">
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex-1 px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-lg hover:from-green-600 hover:to-emerald-600 transition-colors disabled:opacity-50"
          >
            {loading ? 'Adding...' : 'Add Earrings'}
          </button>
        </div>
      </form>
