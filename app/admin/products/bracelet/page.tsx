...
export default function BraceletPage() {
  ...
  const fetchBraceletProducts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const braceletProducts = data.products.filter((p: Product) => p.category === 'bracelet')
        setProducts(braceletProducts)
      }
    } catch (error) {
      console.error('Failed to fetch bracelet products:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const deleteProduct = async (productId: string) => {
    setConfirmModal({
      isOpen: true,
      title: 'Delete Bengals',
      message: 'Are you sure you want to delete this bengals? This action cannot be undone.',
      onConfirm: async () => {
        try {
  ...
  const deleteSelectedProducts = async () => {
    ...
    setConfirmModal({
      isOpen: true,
      title: 'Delete Multiple Bengals',
      message: `Are you sure you want to delete ${selectedProducts.length} selected bengals? This action cannot be undone.`,
      onConfirm: async () => {
        try {
  ...
  const filteredProducts = products.filter(product => {
    ...
    const material = product.productDetails?.bracelet?.material || product.material || ''
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <div className="ml-3 text-gray-600">Loading bracelets...</div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-800">🔗 BENGALS Management</h1>
            <p className="text-gray-600">Manage beautiful bengals and bracelets</p>
          </div>
        </div>
        <Link href="/admin/products/bracelet/add">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <table className="w-full">
            <thead className="bg-gradient-to-r from-purple-50 to-pink-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900">Bengals</th>
                ...
                <tr key={product._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">Bengals</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-2">
                      <Link href={`/admin/products/bracelet/view/${product._id}`}>
                        <button 
                      ...
                      <Link href={`/admin/products/bracelet/edit/${product._id}`}>
                        <button 
        ...
        {paginatedProducts.length === 0 && (
          <div className="text-center py-12">
            ...
            <div className="text-gray-500 mb-2">No bengals found</div>
            <p className="text-sm text-gray-400">Create your first bengals to get started</p>
            <Link href="/admin/products/bracelet/add">
              <button className="mt-4 px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg hover:from-purple-600 hover:to-pink-600">
                Add First Bengals
              </button>
            </Link>
          </div>
      ...
      {filteredProducts.length > 0 && (
        <div className="flex items-center justify-between text-sm text-gray-600">
          <span>Showing {startIndex + 1}-{Math.min(startIndex + productsPerPage, filteredProducts.length)} of {filteredProducts.length} bengals</span>
          <div className="flex items-center space-x-2">
            <button 
